...
public class Automat extends Observable implements Runnable {
	...
	public void removeState(State state) {
		...
		for (int i = 0; i < transis.size(); i++) {
			MainFrame.automat.setChanged(new ChangeEvent(ChangeEventType.TRANSITION_DELETE, transis.get(i), true));
		}
		transis.clear();
		for (State stat : states) {
			...
			for (int i = 0; i < transis.size(); i++) {
				...
				if (state == transi.getFollowerState()) {
					...
					MainFrame.automat.setChanged(new ChangeEvent(ChangeEventType.TRANSITION_DELETE, transi, true));
					// this.setChanged();
		...
		fileIsInSync = false;
		this.setChanged();
		this.notifyObservers(new ChangeEvent(ChangeEventType.STATE_DELETE, state, false));
	}
	...
	public void setActiveState(State activeState) {
		...
		setChanged();
		notifyObservers(new ChangeEvent(ChangeEventType.ACTIVE_STATE_CHANGE, activeState, false));
		if (activeState == null) {
			requestCancel(
					"Bei der Ausführung des Programms kam es zu einer unauthorisierten Übertragung nach Redmond!", true);
		}
	}
	...
	public boolean checkProgramm() {
		...
		if (this.robot == null) {
			JOptionPane.showMessageDialog(MainFrame.mainFrame, "<html>Programm ist mit keinem Roboter verbunden!<br>"
					+ "Stellen Sie zunächst eine Verbindung mit einem Roboter her.</html>",
					"Automat kann nicht ausgeführt werden!", JOptionPane.WARNING_MESSAGE);
		} else if (initialState == null) {
			JOptionPane.showMessageDialog(MainFrame.mainFrame, "<html>Es ist kein Startzustand vorhanden!<br>"
		...
		while (!cancelRequested) {
			...
			for (Transition trans : activeState.getTransitions()) {
				if (trans.eval()) {
					...
					if (MainFrame.DEBUG) {
						System.out.println("Time between State Change: "
					...
					notifyObservers(new ChangeEvent(ChangeEventType.ACTIVE_STATE_CHANGE, activeState, false));
					if (activeState == null) {
						requestCancel("Bei der Ausführung des Programms kam es zu einem Pinguinangriff!", true);
					}
				}
			}
	...
	public void requestCancel(final String reason, boolean abnormal) {
		...
		if (MainFrame.DEBUG) {
			System.out.println("Cancel requested, Grund: " + reason + " abnormal: " + abnormal);
		}
		if (abnormal) {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					JOptionPane.showMessageDialog(MainFrame.mainFrame, reason, "Programmausführung abgebrochen",
	...
	public void connect(String connectTo) {
		...
		try {
			...
			JOptionPane.showMessageDialog(MainFrame.mainFrame,
					"<html>Bei der Initialisierung der Roboters trat ein Fehler auf!<br>" + e.getMessage(),
					"Verbindung konnte nicht hergestellt werden!", JOptionPane.WARNING_MESSAGE);
			return;
		}
		if (MainFrame.DEBUG) {
	...
	public boolean saveAutomat(File file) throws IOException {
		...
		ArrayList<Transition> transitions = new ArrayList<Transition>(states.size() * 3);
	...
	private void saveState(State state, RandomAccessFile saveFile) throws IOException {
		saveFile.writeBytes("<State>\r\n");
		saveFile.writeBytes("<initial> " + state.isInitialState() + " </initial>\r\n");
		saveFile.writeBytes("<label> " + state.getText() + " </label>\r\n");
		saveFile.writeBytes("<location> " + state.getX() + " " + state.getY() + " </location>\r\n");
		for (Action action : state.getActions()) {
			saveFile.writeBytes("<action> " + action.toString() + " </action>\r\n");
		}
		saveFile.writeBytes("</State>\r\n");
	}

	private void saveTransition(Transition transition, RandomAccessFile saveFile) throws IOException {
		saveFile.writeBytes("<Transition>\r\n");
		saveFile.writeBytes("<label> " + transition.getLabel() + " </label>\r\n");
		saveFile.writeBytes("<xOffset> " + transition.getXOffset() + " </xOffset>\r\n");
		saveFile.writeBytes("<rootState> " + states.indexOf(transition.getRootState()) + " </rootState>\r\n");
		saveFile.writeBytes("<followerState> " + states.indexOf(transition.getFollowerState())
				+ " </followerState>\r\n");
	...
	public static Automat loadAutomat(File file) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			if (line.equals("<State>")) {
				...
				while (line != null && !(line = reader.readLine()).equals("</State>")) {
					String[] splitLine = line.split(" ");
					if (splitLine[0].equals("<action>")) {
						if (!splitLine[1].endsWith("CONTROLLER")) {
							actions.add(new Action(splitLine[1], Integer.parseInt(splitLine[2])));
						} else {
							ActionController ac = new ActionController(splitLine[1]);
						...
						location = new Point(Integer.parseInt(splitLine[1]), Integer.parseInt(splitLine[2]));
					}
				}
				...
				while (line != null && !(line = reader.readLine()).equals("</Transition>")) {
					String[] splitLine = line.split(" ");
					if (splitLine[0].equals("<rootState>")) {
						rootState = automat.getStates().get(Integer.parseInt(splitLine[1]));
					} else if (splitLine[0].equals("<followerState>")) {
						followerState = automat.getStates().get(Integer.parseInt(splitLine[1]));
					} else if (splitLine[0].equals("<label>")) {
						label = splitLine[1];
				...
				rootState.addTransition(new Transition(rootState, followerState, label, xOffset, guard));
			}
		}
		reader.close();
